...
class ResumePayment(models.Model):
    ...
    resume = models.ForeignKey(Resume, on_delete=models.CASCADE, related_name='payments')
    razorpay_order_id = models.CharField(max_length=100, unique=True)
    razorpay_payment_id = models.CharField(max_length=100, blank=True, null=True)
    razorpay_signature = models.CharField(max_length=255, blank=True, null=True)
    amount = models.IntegerField(default=2900)  # In paise (2900 = ₹29.00)
    ...
    def __str__(self):
        return f"Payment #{self.id} - Resume {self.resume_id} - {self.status} (₹{self.amount/100})"
